  ...
  switch (action.type) {
    case "ADD_CART": {
      const data = {
        ...
        price: action.payload.data.salePrice
          ? action.payload.data.salePrice
          : action.payload.data.price,
        unitPrice: action.payload.data.salePrice
          ? action.payload.data.salePrice
          : action.payload.data.price,
    ...
    case "DECREASE": {
      ...
      if (newCart[index].qty === 1) {
        const filteredCartItems = newCart.filter(
          (c) => c._id !== newCart[index]._id
        );
